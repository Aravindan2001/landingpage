...
export default function Landing() {
  ...
  const EMAIL = "info@comtechlanka.com";
...
function ServiceIcon({ name, className = "h-6 w-6" }) {
  ...
  switch (name) {
    case "web":
      return (
        <svg viewBox="0 0 24 24" {...props}>
          <path className={common} d="M4 5h16v10H4z" />
          <path className={common} d="M8 19h8" />
          <path className={common} d="M12 15v4" />
          <path className={common} d="M7 8h10" />
        </svg>
      );
    case "mobile":
      return (
    ...
    case "hosting":
      return (
        <svg viewBox="0 0 24 24" {...props}>
          <path d="M6 7h12a3 3 0 0 1 0 6H7a3 3 0 0 1-1-6Z" />
          <path d="M6 17h12a3 3 0 0 0 0-6" />
          <path d="M8 10h.01" />
          <path d="M8 20h.01" />
        </svg>
      );
  ...
  const emailValid = (val) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val.trim());

  function validate(next) {
    const e = {};
    if (!next.name.trim()) e.name = "Full name is required.";
    if (!next.phone.trim()) e.phone = "Phone / WhatsApp is required.";
    if (!next.email.trim()) e.email = "Email is required.";
    else if (!emailValid(next.email)) e.email = "Enter a valid email address.";
...
.hero-card{
  background: rgba(10,10,10,0.70);      /* ✅ stronger card bg */
...
>
  <div className="relative mx-auto flex max-w-7xl items-center px-4 py-3">
    ...
    <div className="ml-auto flex items-center gap-2">
      ...
      {/* ✅ HERO SLIDESHOW (100% working with <img>) */}
    {/* ✅ HERO SLIDESHOW */}
<section
  id="top"
...
>
  ...
  <div className="relative z-10 mx-auto max-w-7xl px-4 py-16 md:py-24 w-full">
    <div className="grid gap-10 lg:grid-cols-12 lg:items-center">
      ...
      {/* ✅ Right quote card (now clearly visible) */}
...
>
  <style>{`
    /* Animated red bar under title */
    .bar-pulse {
      animation: barPulse 1.8s ease-in-out infinite;
      transform-origin: center;
    }
    @keyframes barPulse {
      0%   { transform: scaleX(.55); opacity: .75; }
      50%  { transform: scaleX(1);   opacity: 1; }
      100% { transform: scaleX(.65); opacity: .85; }
    }
    ...
    /* Subtle red glow for each card */
    .card-glow {
      position: absolute;
      inset: -1px;
      border-radius: 24px;
      background: radial-gradient(circle at 20% 10%, rgba(225,6,0,0.10), transparent 55%);
      opacity: 0;
  ...
  {/* ✅ Background red glow overlay (MUST be inside section) */}
  ...
  <div className="mx-auto max-w-7xl px-4">
    ...
    <div data-reveal className="reveal text-center">
      ...
      <div className="mt-3 flex justify-center">
        <div
          className="bar-pulse h-[3px] w-28 rounded-full"
          style={{ backgroundColor: BRAND }}
        />
      </div>
        ...
        >
          <div className="service-card relative h-full rounded-3xl border border-neutral-200 bg-white p-7">
            ...
            <div className="relative flex items-start justify-between">
              {/* Icon */}
              <div
                className="icon-float grid h-12 w-12 place-items-center rounded-2xl text-white shadow-sm"
                style={{
                  background:
                    "linear-gradient(135deg, rgba(225,6,0,1), rgba(225,6,0,0.14))",
                }}
              >
            ...
            <h3 className="relative mt-5 text-base font-extrabold text-neutral-900">
              {s.title}
            </h3>
...
<section id="contact" className="relative overflow-hidden bg-neutral-50 py-16 md:py-24">
  <style>{`
    ...
    .contact-card:hover{
      ...
      box-shadow: 0 18px 60px rgba(0,0,0,.08), 0 12px 40px rgba(225,6,0,.10);
    }

    /* ✅ Inputs: red glow on focus + subtle red hover */
    .contact-input{
      transition: box-shadow .2s ease, border-color .2s ease, background .2s ease;
    }
    .contact-input:hover{
      border-color: rgba(225,6,0,.22);
      box-shadow: 0 0 0 3px rgba(225,6,0,.06);
    }
    .contact-input:focus{
      outline: none;
      border-color: rgba(225,6,0,.45);
      box-shadow: 0 0 0 5px rgba(225,6,0,.14);
    }
    ...
    .contact-btn:hover{
      ...
      box-shadow: 0 16px 45px rgba(225,6,0,.18);
    }

    /* ✅ Small red accent underline animation under headings */
    .contact-underline{
      height: 3px; width: 92px; border-radius: 999px;
      background: ${BRAND};
    ...
    <div className="grid gap-10 lg:grid-cols-12 lg:items-start">
      ...
      <div className="lg:col-span-7">
        <div className="contact-card rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm">
          ...
          {/* small underline under form heading */}
          <div className="mt-3">
          ...
          <form className="mt-6 grid gap-4" onSubmit={onSubmit} noValidate>
            <div className="grid gap-4 sm:grid-cols-2">
              {/* ✅ Custom input wrapper (keeps your validation UI) */}
              <div>
...
function Field({ label, type = "text", value, onChange, error, placeholder }) {
  return (
    <div>
      <label className="text-xs font-extrabold text-neutral-700">{label}</label>
      <input
        type={type}
        value={value}
        ...
        className={[
          "mt-1 w-full rounded-2xl border bg-white px-3 py-2 text-sm outline-none transition",
  ...
  return (
    <div>
      ...
      <textarea
        ...
        placeholder={placeholder}
        className={[
          "mt-1 min-h-[120px] w-full resize-y rounded-2xl border bg-white px-3 py-2 text-sm outline-none transition",
          "focus:ring-4 focus:ring-neutral-100",
          error
            ? "border-red-300 focus:border-red-400 focus:ring-red-100"
            : "border-neutral-200 focus:border-neutral-300",
